    ...
    React.useEffect(() => {
        if (containerRefLocal.current) {
            ...
            Object.keys(pathSelectorToLocationName).forEach((key) => {
                ...
                if (onTouchLocation) {
                    locationPath.addEventListener('touchstart', useTapHook.onTouchStart(() => null))
                    locationPath.addEventListener('touchmove', useTapHook.onTouchMove(() => null))
                    locationPath.addEventListener('touchend', useTapHook.onTouchEnd((event) => {
                        Object.keys(pathSelectorToLocationName).forEach((key) => {
                            const locationPath = svg.querySelector('#' + key)
